...
func TestBuildService_QuickBuildUsesLocalCommitWithoutPull(t *testing.T) {
	...
	sharedPath := filepath.Join(root, "shared-auth")
	require.NoError(t, os.MkdirAll(sharedPath, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(sharedPath, "go.mod"), []byte("module example.com/shared-auth\n"), 0o644))
	repository, err := gitlib.PlainInit(repoPath, false)
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(repoPath, "Dockerfile"), []byte("FROM scratch\nCOPY shared-auth/go.mod ./shared-auth/\n"), 0o644))
	...
	require.NoError(t, err)
	assert.Equal(t, resolvedRoot, captured.ContextDir)
	assert.Equal(t, "AdminService/Dockerfile", captured.Dockerfile)
	...
	assert.Equal(t, "AdminService/Dockerfile", buildRecord.Dockerfile)
	...
	assert.Equal(t, resolvedRoot, captured.ContextDir)
	assert.Equal(t, "plain-service/Dockerfile", captured.Dockerfile)
